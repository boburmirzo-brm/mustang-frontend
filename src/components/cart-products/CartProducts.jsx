  ...
  return (
    <>
      <div className={s.cart_products}>
        {
          PRODUCTS?.map(({_id, title, price, urls, quontity}) => <div key={_id} className={s.product}>
            ...
            <button 
            // onClick={()=> dispatch({type: ADD_TO_CART, payload: cart?.filter(item => item._id !== _id)})}
    ...
    <div className={s.cart_actions}>
      <h4>{PRODUCTS.reduce((a,b) => a+(b.price * b.quontity), 0).brm('int', 0)}</h4>
